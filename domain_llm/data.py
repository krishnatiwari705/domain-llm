...
def prepare_dataset(
    ...
    label_rows = ds[train_split]
    label_map = {row["label"]: row["label_text"] for row in label_rows}
    labels = [label_map[index] for index in sorted(label_map)]
            ...
            records.append(
                {
                    "text": row["text"],
                    "label": label,
                    "training_text": training_text(row["text"], label, labels),
                }
            )
    ...
    return {
        "labels": len(labels),
        "train": len(ds[train_split]),
        "test": len(ds[test_split]),
    }
